...
public class Optional_01_Test {
	...
	public void test_optional_ifPresent() throws Exception {
		...
		// rechercher dans la liste ci-dessus la 1ère personne ayant 18 ans avec la
		// méthode "findFirst"
		Optional<Person> optPerson = persons.stream().findFirst();
		assertThat(optPerson.isPresent(), is(true));

		// afficher la personne en question si l'optional contient une personne
	...
	public void test_optional_notPresent() throws Exception {
		...
		// rechercher dans la liste ci-dessus la 1ère personne ayant 75 ans avec la méthode "findFirst"
		Optional<Person> optPerson = persons.stream().filter(age -> age.getAge() == 75).findFirst();
		assertThat(optPerson.isPresent(), is(false));

		// si la personne n'existe pas, jeter une exception NotPresentException avec la méthode "orElseThrow"
